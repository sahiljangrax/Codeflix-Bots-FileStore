...
class Bot(Client):
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL1:
            try:
                ...
                self.LOGGER(__name__).info("\nBot Stopped. https://t.me/SahilJangradm")
                sys.exit()
        if FORCE_SUB_CHANNEL2:
            try:
                ...
                self.LOGGER(__name__).info("\nBot Stopped.  https://t.me/SahilJangradmt for support")
                sys.exit()
        if FORCE_SUB_CHANNEL3:
            try:
                ...
                self.LOGGER(__name__).info("\nBot Stopped.  https://t.me/SahilJangradm for support")
                sys.exit()
        ...
        try: await self.send_message(OWNER_ID, text = f"<b><blockquote>🤖 Bᴏᴛ Rᴇsᴛᴀʀᴛᴇᴅ by @Lonely_Movies</blockquote></b>")
        except: pass
    ...
    def run(self):
        ...
        self.LOGGER(__name__).info("Bot is now running. Thanks to SahilJangradm")
        try:
            loop.run_forever()
